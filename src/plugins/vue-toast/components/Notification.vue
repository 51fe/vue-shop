...
export default {
  ...
  methods: {
    addToast(option) {
      ...
      // if have onCreated
      if (typeof option.onCreated !== 'undefined') {
        // wait dom update after call cb
        this.$nextTick(() => {
          option.onCreated()
        })
      }
    ...
    removeToast (option) {
      // if have onClosed
      if (option.onClosed !== undefined) {
        option.onClosed()
      }
      this.list.splice(this.list.indexOf(option), 1)
    },

    processObjectData(options) {
      // if Object
      if (typeof options === 'object' && options.msg !== undefined) {
        ...
        if (options.closeOnHover === undefined) {
          options.closeOnHover = this.defaultCloseOnHover
        }
        return options
      }
      // if String
    ...
    error(msg, title) {
      let option = this.processObjectData(msg)
      option['type'] = 'error'
      if (title !== undefined) {
        option['title'] = title
      }
      return this.addToast(option)
    },
